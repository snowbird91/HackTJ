...
let timerTime = WORK_DURATION;
let currentTimerMode = 'work';  // 'work' or 'break'
let pomodoroCycle = 0;          // counts completed work sessions
let timerInterval = null;
let timerRunning = false;

const sessionLabel = getElement('session-label');
...
function resetTimer() {
    pauseTimer();
    timerTime = currentTimerMode === 'work' ? WORK_DURATION : ((pomodoroCycle % 4 === 0) ? LONG_BREAK_DURATION : SHORT_BREAK_DURATION);
    updateTimerDisplay();
}
...
function timerCompleted() {
    ...
    if (currentTimerMode === 'work') {
        // Work session completed: add focus time and award XP
        userStats.focusMinutes = Number(userStats.focusMinutes || 0) + (WORK_DURATION / 60);
        awardXP(25);
        pomodoroCycle++;
        // Use a long break every 4 work sessions; otherwise, a short break
        timerTime = (pomodoroCycle % 4 === 0) ? LONG_BREAK_DURATION : SHORT_BREAK_DURATION;
        currentTimerMode = 'break';
        showNotification('Work session complete! Time for a break.', 'success', 5000);
    } else {
        // Break session complete: start a new work session
        timerTime = WORK_DURATION;
        currentTimerMode = 'work';
        showNotification('Break over! Back to work.', 'info', 5000);
    }
    updateTimerDisplay();
}
...
function updateTimerDisplay() {
    ...
    sessionLabel.textContent = currentTimerMode === 'work' ? "Work Session" : "Break Session";
}
